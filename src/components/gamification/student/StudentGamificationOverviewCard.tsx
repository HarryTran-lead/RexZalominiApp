  ...
  return (
    <div className="rounded-2xl bg-gradient-to-br from-red-600 via-red-500 to-rose-500 p-4 text-white shadow-md">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="rounded-xl bg-white/20 px-2.5 py-1 text-xs font-semibold">Lv. {studentLevel?.level ?? 1}</div>
      </div>

      <div className="mt-3">
        <div className="mb-1 flex items-center justify-between text-[11px] text-red-50">
          <span>XP: {studentLevel?.currentXp ?? 0}</span>
          <span>{studentLevel?.xpRequiredForNextLevel ?? 0} để lên cấp</span>
        </div>
        <div className="h-2 w-full rounded-full bg-white/30">
          <div
            className="h-2 rounded-full bg-white"
            style={{
              width: `${calculateLevelProgress(studentLevel?.currentXp ?? 0, studentLevel?.xpRequiredForNextLevel ?? 1)}%`,
            }}
          />
        </div>
      </div>
